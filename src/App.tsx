...
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean; errorInfo: any }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[100] p-4 font-mono">
          <div className="max-w-md w-full bg-zinc-950 border border-red-900 p-6 rounded shadow-[0_0_50px_rgba(153,27,27,0.3)]">
            <h2 className="text-red-600 font-bold mb-4 flex items-center gap-2">
              <AlertTriangle className="w-5 h-5" /> SYSTEM_CRITICAL_ERROR
            </h2>
            <div className="text-xs text-zinc-400 space-y-2 mb-6">
              <p>An unrecoverable error has occurred in the Azrael core.</p>
              <div className="bg-black p-3 border border-zinc-900 overflow-x-auto">
            ...
            <button 
              ...
              className="w-full py-2 bg-red-900 hover:bg-red-800 text-white text-xs font-bold uppercase tracking-widest transition-colors"
            >
              Reboot_System
            </button>
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
        ...
        const logData = {
          type: "log",
          message: `ARCHITECT_IDENTIFIED: ${currentUser.email}`,
          timestamp: new Date().toISOString(),
          metadata: { uid: currentUser.uid }
        };
        addDoc(collection(db, "system/logs/entries"), logData).catch(e => handleFirestoreError(e, OperationType.CREATE, "system/logs/entries"));
      }
    });
    return () => unsubscribe();
  }, []);

  // Real-time Logs from Firestore
  useEffect(() => {
    if (!isAuthReady) return;

    const q = query(
      collection(db, "system/logs/entries"),
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const newLogs = snapshot.docs.map(doc => {
        const data = doc.data();
        return `[${new Date(data.timestamp).toLocaleTimeString()}] ${data.type === 'breach' ? 'BREACH' : 'PROTOCOL'}: ${data.message}`;
      });
      setLogs(newLogs.reverse());
    }, (error) => {
      // Only handle if it's a permission error, otherwise it might be noisy
      if (error.message.includes("insufficient permissions")) {
        console.warn("Log stream restricted. Admin access required.");
      }
    });
  ...
  const logBreach = async (msg: string) => {
    if (!user) return;
    try {
      await addDoc(collection(db, "system/logs/entries"), {
        type: "breach",
        message: msg,
        timestamp: new Date().toISOString(),
      ...
      handleFirestoreError(e, OperationType.CREATE, "system/logs/entries");
    }
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        const sessionId = "current_session"; // For simplicity, just one session for now
        await setDoc(doc(db, `users/${user.uid}/sessions`, sessionId), {
          sessionId,
          userId: user.uid,
          messages: finalMessages.map(m => ({ ...m, timestamp: new Date().toISOString() })),
          updatedAt: new Date().toISOString()
        }).catch(e => handleFirestoreError(e, OperationType.WRITE, `users/${user.uid}/sessions/${sessionId}`));
      }
        ...
        <header className="relative z-10 border-b border-zinc-800 p-4 flex items-center justify-between bg-black/80 backdrop-blur-md">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-4 text-[10px] uppercase tracking-widest text-zinc-500">
              {user ? (
                <div className="flex items-center gap-3 border border-zinc-800 px-2 py-1 rounded bg-zinc-900/50">
                  <div className="flex items-center gap-2 text-zinc-300">
                    {user.photoURL ? (
                      <img src={user.photoURL} alt="avatar" className="w-4 h-4 rounded-full grayscale" />
                    ) : (
                ...
                <button 
                  onClick={handleLogin}
                  className="flex items-center gap-1 transition-all border border-red-900/50 px-2 py-1 rounded bg-red-950/10 text-red-500 hover:bg-red-900/20"
                >
                  <LogIn className="w-3 h-3" /> Login_Architect
                </button>
              )}
              <button 
                onClick={async () => {
