  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message === "MISSING_OR_INVALID_API_KEY") {
        errorMsg = "PROTOCOL_ERROR: API_KEY_NOT_FOUND. CONFIGURE SECRETS PANEL OR SELECT KEY.";
      ...
      if (Math.random() > 0.95) {
        setBreaches(prev => [
          { 
            ...
            msg: "UNAUTHORIZED_VOID_ACCESS_DETECTED", 
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-4 text-[10px] uppercase tracking-widest text-zinc-500">
            <button 
              ...
              className={`flex items-center gap-1 transition-colors border border-zinc-800 px-2 py-1 rounded bg-zinc-900/50 ${hasAuth ? "text-green-500" : "text-zinc-400 hover:text-red-500"}`}
            >
            ...
            <div className="flex -space-x-2">
              {[1,2,3,4].map(i => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-black bg-zinc-900 flex items-center justify-center overflow-hidden">
                  <img src={`https://picsum.photos/seed/azrael-${i}/32/32`} alt="node" className="w-full h-full object-cover grayscale opacity-50" />
                </div>
              ))}
            </div>
          </section>
